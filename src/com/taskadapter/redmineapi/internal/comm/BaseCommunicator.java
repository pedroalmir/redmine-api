...
public class BaseCommunicator implements Communicator<HttpResponse> {
	...
	public BaseCommunicator(RedmineOptions options) {
		...
		try {
			connManager = HttpUtil.createConnectionManager(options.getMaxOpenConnections());
		...
		if (connManager != null) {
			evictor = new ConnectionEvictor(connManager, options.getEvictionCheckInterval(), options.getIdleTimeout());
			runEvictor(evictor);
		} else {
			evictor = null;
		}
	...
	public <R> R sendRequest(HttpRequest request, ContentHandler<HttpResponse, R> handler) throws RedmineException {
		...
		try {
			...
			throw new RedmineTransportException("Cannot fetch data from " + getMessageURI(request) + " : " + e1.toString(), e1);
		}
	}
